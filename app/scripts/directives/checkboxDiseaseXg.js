define(['directives/directives'],function(directives){
        directives.directive('checkboxDiseaseXg',[function(){
	        return {
	            restrict:"EA",
                scope :{
                    ngModel:"=",
	            ...
	            replace:true,
                link : function(scope){
                    scope.datas = [{id:"arch_xgjb_1",text:"未发现"},{id:"arch_xgjb_2",text:"夹层动脉瘤"},{id:"arch_xgjb_3",text:"动脉闭塞性疾病"},{id:"arch_xgjb_4",text:"其他"}];
                    scope.click = function($event,code){
                        var checkbox = $event.target;
                        code = code + ";";
                        if(checkbox.checked){
                            //------选中的
                            if(code!="arch_xgjb_1;" && scope.ngModel){
                                if(scope.ngModel.indexOf(code)==-1 && scope.ngModel.indexOf("arch_xgjb_1;")==-1){
                                    //------没有的
                                    scope.ngModel += code;
                                }else{
                                	scope.ngModel = code;
