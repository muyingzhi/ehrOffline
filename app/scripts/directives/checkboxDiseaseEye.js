define(['directives/directives'],function(directives){
        directives.directive('checkboxDiseaseEye',[function(){
	        return {
	            restrict:"EA",
                scope :{
                    ngModel:"=",
	            ...
	            replace:true,
                link : function(scope){
                    scope.datas = [{id:"arch_ybjb_1",text:"未发现"},{id:"arch_ybjb_2",text:"视网膜出血或渗出 "},{id:"arch_ybjb_3",text:"视乳头水肿 "},{id:"arch_ybjb_4",text:"白内障 "},{id:"arch_ybjb_5",text:"其他"}];
                    scope.click = function($event,code){
                        var checkbox = $event.target;
                        code = code + ";";
                        if(checkbox.checked){
                            //------选中的
                            if(code!="arch_ybjb_1;" && scope.ngModel){
                                if(scope.ngModel.indexOf(code)==-1 && scope.ngModel.indexOf("arch_ybjb_1;")==-1){
                                    //------没有的
                                    scope.ngModel += code;
                                }else{
                                	scope.ngModel = code;
